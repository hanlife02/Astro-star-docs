...
Changes under `Astro-star/docs/` are checked in the main repository first. A successful documentation build sends a `deploy-docs` repository dispatch event containing the source commit SHA. This repository then checks out that exact Astro-star revision, builds `docs/`, and publishes the result with GitHub Pages.
